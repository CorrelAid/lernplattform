...
3. Gibt es einen statistischen zusammenhang zwischen der Anzahl an Events und der Menge an Plastik?
...
Ein sehr nützliche Kombination ist: `group_by()` und `summarize()`. Damit können wir Daten gruppieren und Kennzahlen wie den Mittelwert und die Standardabweichung der gefundenen Plastikstücke ausrechnen lassen.

```{r Gruppieren Statistik und Statistik ausgeben 1}
...
```{r Gruppieren Statistik und Statistik ausgeben 2}
...
```{r audit Perspektive: nur Europa und Asien}
...
```{r anzahl plastikarten}
audit_plastic3 <- audit_plastic_long %>%
  group_by(country) %>%
  mutate(n_types=sum(!is.na(amount)))

```

```{r durchschnittliche Anzahl plastikarten je Kontinent}
audit_plastic3  %>%
  group_by(continent) %>%
...
<!-- ```{r plastikarten je Kontinent aufsummieren} -->
<!-- audit_plastic3 <- audit_plastic_long %>% -->
...
<!-- audit_plastic3 <- audit_plastic_long %>% -->
<!--   group_by(continent, type) %>% -->
<!--   summarize(amount_sum=sum(amount, na.rm = TRUE), -->
<!--             grand_total_sum = sum(grand_total, na.rm=TRUE)) %>% -->
<!--   ungroup() -->

<!-- ``` -->

<!-- ```{r audit Perspektive: relative Häufigkeiten und danach sortieren} -->

<!-- audit_plastic3<- audit_plastic3 %>% -->
<!--    mutate(rel_frequency = amount_sum / grand_total_sum) %>% -->
<!--    arrange(rel_frequency) -->

<!-- ``` -->

<!-- ```{r audit Perspektive: nach relative Häufigkeiten sortieren} -->

<!-- audit_plastic3<- audit_plastic3 %>% -->
<!--    #group_by(continent) %>% -->
<!--    arrange(continent, rel_frequency) -->

<!-- ``` -->


<!-- ```{r top3 identifizieren} -->
<!-- audit_plastic4<- audit_plastic2 %>% -->
<!--   group_by(continent, type) %>% -->
<!--   summarize(amount_sum=sum(amount, na.rm = TRUE)) %>% -->
<!--   slice_max(order_by = amount_sum, n = 3) -->
...
1. ZUr Diskussion: Welche Fragen möchtet ihr den Daten noch stellen? Wie könnte eine Visualisierung oder eine zusammenfassende Statistik dabei helfen?

2. Visualisierunsübung
