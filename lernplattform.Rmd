...
library(purrr)
library(RSQLite)
#flipbookr, echarts4r,wesanderson, janitor,rstatix,haven, DBI, ggthemes, BH, anytime,gtrendsR
...
reduced_2019 <- plastics_2019 %>%
  dplyr::filter(parent_company != "Grand Total" & country != "EMPTY") %>%
...
<!--## Datentransformation -->
```{r results='asis'}
#out <- knitr::knit_child("07_datentransformation_review.Rmd", quiet = TRUE)
#cat(out, sep = '')
```
...
```{r results='asis'}
#out <- knitr::knit_child("09_grundlagenderstatistik.Rmd", quiet = TRUE)
#cat(out, sep = '')
```
