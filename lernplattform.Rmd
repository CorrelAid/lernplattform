...
audit_db <- plastics_prep %>%
  ...
  pivot_longer(c("hdpe", "ldpe", "o", "pet", "pp", "ps", "pvc"), names_to = "plastic", values_to = "count") %>% 
  select(countrycode, year, parent_company, plastic, count)


con <- dbConnect(RSQLite::SQLite(), "docs/daten/plastics.sqlite")
dbWriteTable(con, "audits", audit_db, overwrite = TRUE)
dbWriteTable(con, "events", community_db, overwrite = TRUE)
dbWriteTable(con, "countries", country_db, overwrite = TRUE)
dbListTables(con)
dbDisconnect(con)
```
