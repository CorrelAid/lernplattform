...
quiz(caption = NULL,
  ...
  question("Warum sind Boxplots so nützlich?",
  ...
  question("Warum haben Kuchendiagramme einen so schlechten Ruf?",
    ...
    correct = "Richtig! Kuchendiagramme sollten nur für Variablen mit zwei bis drei Kategorien erstellt werden, z.B. Ja, Nein, NA. Bei mehr Kategorien können wir Verhältnisse nicht mehr richtig abschätzen.",
    incorrect = "Leider falsch: Kuchendiagramme sollten nur für Variablen mit zwei bis drei Kategorien erstellt werden, z.B. Ja, Nein, NA. Bei mehr Kategorien können wir Verhältnisse nicht mehr richtig abschätzen.",
    allow_retry = TRUE,
    try_again_button = "Nochmal versuchen"
  ), 
  question("Wie heißt die erste Empfehlung von From-Data-to-Viz für mehrere geordnete, numerische Variablen?",
    answer("Boxplots"),
...
In dieser Sitzung lernt Ihr verschiedene Visualisierungsformen- und Möglichkeiten mit `ggplot2` kennen. Ziel ist es, dass Ihr hinterher die Visualisierungen in unserem Report und in unserer Shiny Web App erstellen könnt ([zur App](https://correlaid.shinyapps.io/breakfreefromplastic/){target="_blank"}. 

#### Wiederholung: `ggplot2`-Basics
Erinnert Ihr Euch noch an die fünfte Sitzung? 
...
- Für einen ganz simplen Plot benötigen wir immer:
    ...
    - `geom_xxx()` (die **geometrische Form**, d.h. mit welcher Visualisierungsform Eure Daten dargestellt werden sollen z.B. in einem Balkendiagramm oder Histogramm) 
...
#### Initialisierung
Nun bauen wir diese Graphik Schritt-für-Schritt nach. Wir beginnen mit dem Grundgerüst einer `ggplot`-Graphik: dem **Erstellen eines `ggplot`-Objekts und der Definition von ästhetischen Attributen**. Das sind die Variablen bzw. Spalten, die auf der x-und y-Achse dargestellt werden sollen. Was produziert dieser Code? Was passiert, wenn Ihr die Variablen vertauscht?
...
Das Schöne an der `ggplot2` Logik ist, dass man mit wenigen Handgriffen Visualisierungstypen austauschen kann. 

Nun fügen wir dem Plot die **Datenpunkte** hinzu, um einen ersten Scatterplot (hier: Beeswarmplot) zu erstellen. Diese Codezeile unterscheidet sich je nach gewählter Visualisierungsform.
...
#### Layoutoptionen
Mit **Layoutoptionen** schaffen wir mehr Übersichtlichkeit. Mit **`alpha = Wert`** ( zwischen 0 und 1) werden die Punkte etwas transparenter, mit **`size= Wert`** (Wert zwischen 0 und +∞) etwas größer und so auch einzeln besser sichtbar.
...
```
#### Achsenbegrenzungen
Auch in Visualisierungen können wir **Extremwerte** exkludieren, wenn die Graphik sonst stark an Leserlichkeit verliert. Idealerweise begrenzt Ihr Koordinatensysteme nicht mit absoluten Werten, sondern orientiert Euch an relativen, statistischen Kennzahlen wie Mittelwert und Median multipliziert mit einem Faktor und der Standardabweichung, Varianz oder dem Interquartilsabstand. So bleiben Eure Achsen reaktiv, wenn sich die zugrundeliegenden Daten ändern. <br>
...
#### Themes
Mit **`theme_()`** geben wir unserem Plot-Layout den letzten Schliff: Hier entscheiden wir uns für `theme_minimal()`. Da wir die Standardeinstellungen dieses Themes überschreiben, um mit `theme()` die Legende auszublenden, müssen wir zunächst das Template referenzieren und dann die individuellen Einstellungen vornehmen:
...
Dieser Befehl erlaubt es Euch, statische Visualisierungen zu speichern. Für interaktive Visualisierungen wie beispielsweise die durch `leaflet` oder `echarts4r` erstellen Karten, bieten sich andere Formate wie `.html` (über Rmds) oder aber auch eine ShinyApp an -- mehr dazu erfahrt Ihr in den nächsten Wochen!

### Exkurs: Karten mit `leaflet`
Ihr habt bereits gelernt, wie man **geographische Daten** importiert. Wir fügen die sog. Polygone unserem `community`-Datensatz an und fügen drei Spalten ein, die uns später bei den Labels hilft (ein toller Trick, den Ihr Euch für andere Visualisierungsprojekte merken solltet).
...
Neben `ggplot2` und `leaflet` gibt es noch ein drittes R-Package, das wir an dieser Stelle erwähnen möchten: `echarts4r`.

Die Programmierlogik in `echarts4r` unterliegt wie `ggplot2` einer Schichtlogik und funktioniert über die Pipe `%>%`, die Ihr aus dem `tidyverse` kennt. Die Visualisierungen sind hier interaktiv, es gibt z.B. Tooltips (zusätzliche Informationen), die angezeigt werden, wenn Betrachter:innen mit der Maus über die Visualisierung fahren. Ab einem bestimmten Level müsst Ihr zur Bedienung von `echarts4r` allerdings Javascript können, weshalb wie dieses tolle Visualisierungstool hier nur am Rande erwähnen möchten.


### Und jetzt Ihr
Diesmal werden wir Euch kein Rmd vorgeben - es ist Zeit zu testen, was Ihr gelernt habt! Generiert aus den **Top10-Herstellern**, die wir im letzten Kapitel aus dem Datensatz gefiltert haben, ein **Bardiagramm**. In unserer [Beispielapplikation](https://correlaid.shinyapps.io/breakfreefromplastic/){target="_blank"} könnt Ihr die Visualisierung live sehen und nachbauen. Als Layoutoptionen haben wir die Farbe `#4E97AC` und `theme_minimal()` genutzt. Experimentiert gerne mit weiteren Gestaltungsoptionen. Wie würden wir beispielsweise Beschriftungen für die Datenpunkte hinzufügen?
...
- [Visual Inference with R](https://www.mzes.uni-mannheim.de/socialsciencedatalab/article/visinference/){target="_blank"} (engl.)
