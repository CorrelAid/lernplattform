...
- Besonders oft werden folgende Funktionen genutzt: 
    ...
    - `mean()`: arithmetische Mittel, oder schlicht **Mittelwert** oder Durchschnitt
...
quiz(caption = NULL,
  question("Welche Aussagen sind wahr?",
    ...
    answer("Es ist Visualisierungen zu erstellen, um Aussagen über die Bedeutung erhobener Daten zu treffen."),
    answer("Kontext ist bei rigoroser Datenanalyse nicht so wichtig."),
    answer("Die Berechnung statistischer Kennzahlen, die Visualisierung und die Kontextualisierung sind eine wichtige Grundlage (!) für die Interpretation von Daten.", correct = TRUE),
    correct = "Richtig!",
    incorrect = "Leider falsch. Die Berechnung statistischer Kennzahlen, die Visualisierung und die Kontextualisierung sind eine wichtige Grundlage (!) für die Interpretation von Daten.",
    allow_retry = TRUE,
...
Warum diese statistischen Kennzahlen so nützlich sind, lässt sich anhand eines einfachen Beispiels verdeutlichen: Als Lehrer:in schreibt Ihr mit Eurer Klassen einen Test, bei dem Eure Schüler:innen maximal 100 Punkte erreichen und mit 50 bestehen konnten. Die Noten verteilen sich so:

```{r schulnoten, exercise = TRUE}
# Ergebnisse speichern
schulnoten <- c(50, 45, 40, 50, 100, 100, 20, 30, 50, 50)

# Lage- und Streuungsparameter bestimmen
mean(schulnoten)
median(schulnoten)
max(schulnoten) - min(schulnoten)
var(schulnoten)
sd(schulnoten)
```

Ihr rechnet - wie es so üblich ist - den Durchschnitt aus: `r mean(schulnoten)`. Im Durchschnitt mit ein paar Punkten Puffer bestanden! Ziemlich gut und kein Grund zur Sorge... Oder? 

Wenn wir uns den Median anschauen, der die wahre Mitte einer Verteilung angibt und robust gegen Ausreißer ist, fällt uns auf: `r median(schulnoten)` - so viel Puffer gab es gar nicht. Am Häufigsten gab es die Punktzahl 50 (Modalwert) und die Streuung um den Mittelwert ist recht groß: Im Mittel weichen Noten um `r round(sd(schulnoten),0)` Punkte ab und schwanken über eine Spannbreite von `r max(schulnoten) - min(schulnoten)` - fast über die gesamtmögliche Breite. Zwar gab es zwei hohe Punktzahlen, aber der Rest der Klasse hat nicht so gut abgeschnitten. Immerhin sind sogar 40 Prozent der Schüler:innen durch die Prüfung gefallen! Da würden wir uns definitiv Sorgen machen.
...
Um **Fehlschlüsse aus statistischen Kennzahlen zu vermeiden**, solltet Ihr also **verschiedene Lage- und Streuungsparameter bestimmen**, die **Verteilung** Eurer Daten genauer anschauen und die Daten in den richtigen **Kontext** setzen. Und das machen wir jetzt - für einen Datensatz aus dem echten Leben.
...
- [Basic Statistics](https://www.coursera.org/learn/basic-statistics){target="_blank"} auf Coursera (engl.)
- Die sehr leicht verständliche Zusammenfassung zu Lageparameter und Streuungsmaßen auf [Scribbr](https://www.scribbr.de/statistik/deskriptive-statistik/){target="_blank"}