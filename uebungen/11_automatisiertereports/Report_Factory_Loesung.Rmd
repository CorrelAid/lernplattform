...
Rendert ein .Rmd Eurer Wahl mit der `rmarkdown::render()` Funktion, anstatt des 🧶 Knit-Buttons. Ihr könnt den Code dafür in diesem Dokument sammeln. Hier ist sozusagen Eure Report-Factory, in der ihr den Code schreibt, der dann die Reports generiert. 
...
- setzt dort den Namen fest, den Euer Report später haben soll  
```{r}
rmarkdown::render(
...
alternativ können die Inputs auf in den Parametern definiert werden. Dann kann auf den define-input code chunk verzichtet werden.  
...
Was passiert wenn du das Argument `output_dir` verwendest? Schaue im Hilfe-Abschnitt der render Funktion nach diesem Argument und probiere es dann mit dem untenstehenden Code Block selbst aus. Was spassiert, wenn ihr das `output_dir` Argument etwas ändert?
...
Welche Parameter könnt Ihr dort entdecken? 
...
> Einmal ist der Parameter `include_code` als TRUE definiert und einmal FALSE. Dadurch ändert sich die Art und Weise wie der Report gerendert wird. EInmal wird der Code abgedruckt und einmal nicht. 

- Findet ihr auch die Stelle im R Markdown Dokument, der zu dem Ergebnis beigetragen hat?

> im R-Markdown Dokument wird der Parameter den wir verändert haben in Zeile 16 im YAML Abschnitt definiert und dann in Zeile 24 im setup Code-Chunk eingesetzt. 
...
rmarkdown::render(
  ...
  output_file = "Übungsreport-Europe-kein_Code",
  output_format = "html_document",
...
rmarkdown::render(
  input = here::here("09_Report_Plastic.Rmd"),
  output_file = "Übungsreport-Europe-mit_Code",
  output_format = "html_document",
  params = list(highlight_continent = "Europe",
                include_code = TRUE)
  )
...
purrr::walk(
  ...
  .f = ~ rmarkdown::render(
    ...
    output_format = "pdf_document",
    params = list(highlight_continent = .x) # hier wird der Kontinentname übergeben
  )
)
```

Probiere mit dem gleichen Prinzip, zwei Reportsversionen für das Anzeigen von Code zu generieren, dass durch den Parameter `include_code` gesteuert wird. Schreibe also Code der an den verschiedenen Optionen für den Parameter `include_code` entlang "laufen" wird.
...
purrr::walk(
  ...
  .f = ~ rmarkdown::render(
    ...
    output_format = "pdf_document",
    params = list(include_code = .x) # hier werden die beiden Optionen übergeben
  )
)
```
...
Was, wenn wir Reports für alle Varianten von Kontinent-Highlights und jeweils mit und ohne Code erstellen wollen? Auch das geht:

zuerst erstellen wir einen Vektor der alle Kombinationen aus unseren beiden Options-Listen enthält (einmal Kontinente und TRUE/FALSE)
...
Das ist quasi die Anleitung für unsere factory, welche Reports mit welchen Optionen generiert werden sollen. An dieser Anleitung "läuft" unser code wieder entlang, siehe die walk-Funktion:

Diesmal nutzen wir allerdings `pwalk`. Mit dieser Funktion können wir mehrere Argumente gleichzeitig übergeben. Diese Argumente haben wir in unserem Objekt `option_combinations` vorbereitet. 

Wo welche Option eingesetzt wird bestimmen wir mit `.x` und `.y`. Dabei steht `.x` in unserem Fall für `continent_option` und `.y` für `code_option`
...
purrr::pwalk(
  ...
  .f = ~ rmarkdown::render(
    ...
    output_format = "html_document",
    params = list(
      highlight_continent = .x, # hier wird der Kontinentname übergeben
      include_code = .y) # hier werden die beiden Optionen übergeben
  )
)
```
