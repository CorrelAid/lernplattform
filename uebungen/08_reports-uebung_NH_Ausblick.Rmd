...
### Wirkung
Insgesamt wurden auf den **`r sum(community19_by_country$n_events)` Events** mit durchschnittlich **`r as.integer(round(mean(community19_by_country$n_volunteers),0))` Freiwilligen** `r round(mean(community19_by_country$n_pieces),2)` Plastikstücke gesammelt. Das entspricht **`r as.integer(sum(community19_by_country$n_pieces))` Plastikstücken**. 
...
ggplot(data = community19_by_country, aes(n_volunteers, n_pieces, label = country)) + # Initialisierung des ggplots mit Variablen
