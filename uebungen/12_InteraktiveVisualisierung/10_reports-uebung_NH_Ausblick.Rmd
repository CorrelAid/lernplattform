...
### Ausblick
Die Anzahl der Events korreliert nur sehr schwach mit der Anzahl gesammelter Plastikstücke (Korrelation: `r round(cor(community$n_events, community$n_pieces), 2)`). In Ländern mit mehr Freiwilligen wurden zudem mehr Plastikstücke gesammelt. Über den Zusammenhang zwischen der Anzahl der gesammelten Plastikstücke und den Freiwilligen können wir auf Grund der Aggregation der Daten allerdings keine statistische Aussage treffen. Spannend könnte hier deshalb eine genauere Betrachtung dieses Zusammenhanges sein. Es ist jedenfalls auffällig, dass in einigen Ländern viele Freiwillige zusammenkamen, aber nur wenig Plastikstücke gesammelt wurden. Warum das so ist, sollte in Gesprächen mit Organisator:innen geprüft werden. In Betracht kommen hier Fehler beim Reporting, das für die politische Arbeit der Bewegung allerdings zentral ist, oder Events, in denen das Sammeln von Plastik nicht im Vordergrund stand (beispielsweise da sie der Aufklärung dienten).
...
ggplot(data = community, aes(n_events, n_pieces, label = country)) + # Initialisierung des ggplots mit Variablen
  ...
  coord_cartesian(xlim = c(0, median(community$n_events) + 0.5 * sd(community$n_events)), ylim = c(0, median(community$n_pieces) + 0.5 * sd(community$n_pieces))) + # Festlegung der Achsenlänge der x- und y-Achse
  geom_text(size = 2) + 
  labs(
