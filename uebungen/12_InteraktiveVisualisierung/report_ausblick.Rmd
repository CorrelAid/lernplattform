...
polygons_welt <- sf::st_read("https://raw.githubusercontent.com/CorrelAid/lernplattform/q2_review/daten/geospatial/ne_50m_admin_0_countries.shp")
...
Insgesamt wurden auf **`r sum(community$n_events)` Events** mit durchschnittlich **`r as.integer(round(mean(community$n_volunteers),0))` Freiwilligen** im Schnitt `r round(sum(community$n_pieces)/sum(community$n_events),0)` Plastikstücke gesammelt. Das entspricht insgesamt **`r as.integer(sum(community$n_pieces))` Plastikstücken**. 
