...
Einer ähnlichen Logik folgend könnt Ihr mit `ggplot2` Eure Plots auch individuell gestalten. Wie Ihr in Sitzung 5 schon gesehen habt, kann man zum Beispiel die Beschriftungen ändern. `ggplot2` bietet aber noch viel mehr! Ihr könnt Eure Visualisierung individuell gestalten, indem Ihr Farben oder auch Darstellungstypen ändert.
...
```{r, out.width='100%', out.height='100%'}
knitr::include_url("daten/flip_for_correlaid/flip_for_correlaid_color.html")
```
...
Eine weitere beliebte Möglichkeit Eure Visualisierung zu verschönern sind sogenannte `themes` in `ggplot2`. Die `themes` sind unglaublich vielseitig und bieten Euch die Möglichkeit alles so anzupassen, wie ihr es möchtet. Um die `themes` komplett zu verstehen würden wir eine weitere Sitzung dazu benötigen. Daher hier die wichtigsten Informationen in Kürze:
