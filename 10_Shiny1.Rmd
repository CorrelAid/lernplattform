...
community %>% 
  e_charts(country) %>% 
  e_map(volunteers) %>% 
  e_visual_map(volunteers)
```

**Achtung:** Anders als in `ggplot2` nutzen wir jetzt wieder die "pipe" (`%>%`), um die einzelnen Befehle miteinander zu verbinden.
...
1. Überlegt Euch, welche Visualisierung Ihr Euch mit den Daten vorstellen könnt. Schaut Euch zur Inspiration auf [Data to Viz](https://www.data-to-viz.com/){target="_blank"} um und wählt eine Visualisierung, die Ihr mit Euren Daten gerne nachbauen würdet. 
