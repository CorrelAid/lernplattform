...
quiz(
  ...
  question("Welche der folgenden Fragestellungen können mit Daten beantwortet werden:",
    answer("Was macht unsere gesellschaftliche Herausforderung aus?",
      correct = TRUE
    ),
    answer("Wie können wir bessere Entscheidungen treffen?",
      correct = TRUE
    ),
    answer("Wie können wir effizienter und effektiver arbeiten?",
      correct = TRUE
    ),
...
# Das aktuelle Datum bekommen wir mit der Funktion "Sys.Date()", das wir dann 
# formatieren, damit es so aussieht, wie wir das in Deutschland gewohnt sind. 
# %d formatiert den Tag, %m den Monat und %y das Jahr. 
# Als Trennungszeichen haben wir uns für "." entschieden.
datum <- format(Sys.Date(), "%d.%m.%y")

paste("CorrelAid e.V. hat (Stand: ", datum, ") ",
  abgeschlossene_projekte, " Projekte durchgeführt. ",
  aktive_projekte, " sind noch aktiv.",
  sep = ""
)
```
...
Nicht mit allen **Datentypen** können alle Technologien genutzt werden. 
Kennt Ihr bereits die verschiedenen Datentypen, mit denen wir arbeiten? 

Mit der Funktion **class(Euer Objekt)** könnt Ihr überprüfen, welches Datenformat
bei Eurem Objekt vorliegt. Testet die Funktion in Zeile 15 für Datumsformate und
führt den Code aus.

```{r datentypen, exercise=TRUE}
# Hallo ist Text - bei R bezeichnet man dieses Datenformat als "character"
# man erkennt diesen Datentyp an den Anführungszeichen.
class("Hallo")

# 20.5 ist eine Zahl - bei R bezeichnet man Zahlen als "numeric". Manchmal wird
# auch noch zwischen ganzen Zahlen - "integer" - und Zahlen mit Nachkommastellen
# - "double" unterschieden.
class(20)

# Bei der Datenanalyse kommen häufig Vergleiche zum Einsatz, z.B. 
# "zeige nur alle Mitglieder, die **älter sind als** 20". Es werden dann nur alle
# Beobachtungen angezeigt für die dieser Vergleich wahr ist. Solche Vergleiche und
# andere "Tests" werden als logische ("logical") Variablen abgebildet - bei R 
# kann das "TRUE" (zu dt. richtig) und "FALSE" (zu dt. falsch) sein.
alter <- 22
print(alter > 20)
class(alter > 20)

# Bei Datumsformaten geht R besonders klug vor. 
# Testet die Funktion "class()" mit dem Datum. Was fällt Euch auf?
datum <- Sys.Date()
typeof(datum)
# Hier Euer Code
```
