...
Jetzt seid Ihr an der Reihe! Eben habt Ihr den Datensatz bereits gefiltert mit `dplyr::filter()`, jetzt sollt Ihr zusätzlich noch die Variable **country** bereinigen. Hierbei sollen die Länderbezeichungen vereinheitlicht werden, das geht über `dplyr::case_when()`, welches so funktioniert, dass WENN eine Variable einen bestimmten Wert hat, dieser DANN umbenannt wird (WENN ~ DANN). Der Teil des Codes ist hier bereits für Euch schon geschrieben, könnt Ihr noch die fehlenden Funktionen und Variablennamen an Stelle der ??? ergänzen?
...
plastics %>%
  ...
  dplyr::filter(country != "EMPTY",
                parent_company == "Grand Total") %>%
  # ...und bereinigt zwei der Länderbezeichnungen!
  dplyr::???(
    ??? = dplyr::case_when(
