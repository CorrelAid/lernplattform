...
Berechnet mit den Funktionen `dplyr::group_by` und `dplyr::summarise` die Gesamtzahl der Plastikstücke, die pro Kontinent gesammelt wurden. Nutzt dafür den `community`-Datensatz.
...
Neben Zeilen bzw. Beobachtungen könnt Ihr natürlich auch **Spalten bzw. Variablen filtern**. Auch hier hilft Euch das `dplyr` Package mit der Funktion `select` weiter. Mit `select` könnt Ihr entweder die Variablen auswählen, die im Datensatz enthalten sein sollen oder aber mit einem `-` die Variablen auswählen, die aus Eurem Datensatz gelöscht werden sollen. Der folgende Code löscht so zum Beispiel die Variable `grand_total` aus dem `data_raw`-Datensatz:
...
Ups - Ihr erhaltet ein NA? R kann den Wert nicht berechnen, da in der Variable mehrere NAs enthalten sind. Versucht hier zu berechnen, wie viele NAs die Variable hat. <br>
