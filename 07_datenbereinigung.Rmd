...
### comments Phil
- first things first: mega Lektion!
- Anmerkung zu filter: sind die verschiedenen Variablentypen klar genug?
  - Warum brauche ich Anführungszeichen um die 2000, bzw wann brauch ich die nicht?
- Anmerkung zu select: es können auch mehrere Variablen mit `-c()` gelöscht werden (manchmal ganz handy) 
- Anmerkung zu Bild von joins: Zumindest auf meinem Bildschirm war das grau nur sehr schwer zu erkennen
- Anmerkungen zu joins: Variablen müssen nicht unbedingt gleich heißen, vllt eine nützliche Info (dann sieht die by Bedingung etwas anders aus)
- pivoting wäre sehr cool mit Visualisierung im Video, fällt am Anfang extrem schwer sich das vorzustellen
- `str_replace()` könnte man auch mit `str_remove()` ersetzen (oder beide zeigen)
- das ist alles jammern auf höchstem Niveau btw. :D 
...
``` {r 07mean_na2, exercise = TRUE}
# Hier Euer Code!

```
Ups - Ihr erhaltet ein NA? R kann den Mittelwert hier nicht einfach berechnen, da in der Variable mehrere NA's enthalten sind. Versucht hier mal zu berechnen, wie viele NA's die Variable tatsächlich hat. (Hinweis: Experimentiert hier mal mit der Funktion `summary()`):
...
Trotzdem wollt Ihr Euren Mittelwert ja berechnen - dafür könnt Ihr R explizit sagen, dass die NA's bei der Berechnung ignoriert werden sollen:
...
Verschafft euch zuerst einen kleinen Überblick über den Datensatz `wb_areas`: 
