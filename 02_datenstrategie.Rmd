...
- Projekte vom Zweck (und nicht von der Technologie aus) denken
- Auch Technologien mit niedrigem analytischen Reifegrad in Erwägung ziehen
- Quantiative und qualitative Forschungsmethoden nutzen
- Externe und operationale Daten vor dem Beginn von Datenerhebungen evaluieren
- Manuelle Prozesse mit menschlichen Fähigkeiten zu mappen, um passende Technolgien finden
- Mit Prototypen und agilem Arbeiten in Iterationen die Nützlichkeit von Projekten überprüfen

**Was wir vermeiden sollten:**

- (Einmalig) identifizierte Signifikanz überbewerten und Erkenntnisse ohne Sinnhaftigkeitsprüfung übernehmen
- Unsinnige Vergleichsgruppen bilden
- Nur nach präferierten Resultaten suchen
- Voraussetzungen der Datenqualität und -aktualität missachten
- Uns auf Trendtechnologien und BuzzWords fokussieren
- Den Aufwand von Datenprojekten (insb. im Bereich der Organisationskultur) unterschätzen
...
Mit der Funktion **typeof(EureVariable)** könnt Ihr überprüfen, welches Datenformat bei Eurer Variable vorliegt. Testet die Funktion in Zeile 12 für Datumsformate und führt den Code aus.
...
# 20 ist eine Zahl - bei R bezeichnet man numerische Werte als "double", ganze Zahlen als "integer". 
typeof(20)

# Mit Funktionen wie "is.numeric(EureVariable) könnt Ihr ebenfalls überprüfen welches Datenformat vorliegt. Output der Funktion ist eine Variable vom Typ "logical" - bei R kann das "TRUE" (zu dt. richtig) und "FALSE" (zu dt. falsch) sein.
typeof(is.numeric(20))

# Bei Datumsformaten geht R besonders klug vor. Testet hier die Varibale "datum" mit "typeof()". Was fällt Euch auf?
datum <- Sys.Date()
# Hier Euer Code
class(datum)
```
