...
Mit der Funktion **class(EureVariable)** könnt Ihr überprüfen, welches Datenformat bei Eurer Variable vorliegt. Testet die Funktion in Zeile 14 für Datumsformate und führt den Code aus.
...
# 20.5 ist eine Zahl - bei R bezeichnet man Zahlen als "numeric" (manchmal wird auch noch unterschieden zwischen ganzen Zahlen - "integer" - und Zahlen mit Nachkommastellen - "double"). 
class(20.5)

# Bei der Datenanalyse kommen häufig Vergleiche zum Einsatz, z.B. "zeige nur alle Mitglieder, die **älter sind als** 20". Es werden dann nur alle Beobachtungen angezeigt für die dieser Vergleich wahr ist. Solche Vergleiche und andere "Tests" werden als "logical" Variablen abgebildet - bei R kann das "TRUE" (zu dt. richtig) und "FALSE" (zu dt. falsch) sein.
alter <- 22
print(alter > 20)
class(alter > 20)

# Bei Datumsformaten geht R besonders klug vor. Testet hier die Varibale "datum" mit "class()".
datum <- Sys.Date()
# Hier Euer Code
```
