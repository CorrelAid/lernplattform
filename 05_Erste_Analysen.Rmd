...
Agenda der Einheit
0. Vorbereitung: Einlesen und finaler Datencheck
...
table(is.na(audit_plastic))

summary(is.na(audit_plastic))
...
Im Video nehmen wir die *Community-Perspektive* ein und fragen: Wo haben sich wie viele Freiwillige an *Breaking Free From Plastic* beteiligt? Das wäre eher eine  Dafür visualisieren wir vom Datensatz `community` die `aes()`-Variablen continent und volunteer mit `geom_point` in einem Punktediagramm (englisch: Scatterplot).
...
ggsave(here("community_scatterplot.jpg"))
...
Den letzten Schliff können wir dem Layout geben, in dem wir den Stil über `theme_()` ändern. Zum Beispiel mit `theme_minimal()`. Zu dem können wir mit `reorder()` die Kontinente nach Teilnehmerzahl sortieren.
...
- Besonders oft nutzt man: 
    - `min()` für die minimale Ausprägung einer Variabeln
    - `max()` für die maximalen Ausprägung einer Variabeln
...
Wir nutzen diese Kennzahlen, um die folgenden Fragen zu beantworten:
...
Kennzahlen können auch einzeln berechnet werden mittel `summarize`. Dies hat den Vorteil, dass ein kompaktes R-Objekt entsteht (ein `Tibble`), das wir abspeichern könnten. 
...
Weitere interessante Kennzahlen sind: der Median (`median()) und die Länderanzahl (`n()`). Ergänzt den Code um diese beiden Kennzahlen. 
...
- [r4ds](https://r4ds.had.co.nz/){target="_blank"} 
