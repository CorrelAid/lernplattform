...
![*Video: Erste Datenanalysen in R (30min)*](https://youtu.be/wVNVZAM7sV8)
...
- Die wichtigsten Schichten (Bestandteile) eines ggplots sind: 
    ...
    3. `geom_*()` - die **geometrische Form**, mit welcher die Werte dargestellt werden, z.B. geom_bar() für Bardiagramme
...
- Darüber hinaus können mit dem **`tidyverse`** folgende Operationen vorgenommen werden:
...
- Ein besonderes **Power-Duo** ist dabei das Zusammenspiel von `dplyr::group_by()` & `dplyr::summarize()` - so können statistische Kennzahlen **pro Variablenkategorie** (z.B. Land) ausgegeben werden
- Verknüpft werden Operationen des *`tidyverse`* mit dem **Pipe-Operator "%>%"** (zu dt. Rohrbetreiber), der eine ähnliche Funktion wie das "+" in `ggplot2` erfüllt erfüllt.
...
### Übung
#### Überblick
In der letzten Lektionen habt Ihr bereits erfahren, dass wir  in diesem Kurs mit einem [Datensatz](https://github.com/rfordatascience/tidytuesday/tree/master/data/2021/2021-01-26){target="_blank"} von **"Break Free From Plastic"** aus den Jahren 2019 und 2020 arbeiten. Da es in dieser Einheit darum geht, in den **Analyse-Spirit** zu kommen und R als Analysetool kennenzulernen, haben wir den Datensatz "Break Free from Plastic" bereits **bereinigt** und in **zwei Datensätze** aufgeteilt: Community (als `community` hinterlegt) & Audit (als `audit_plastic` hinterlegt). 

Insbesondere bei Datensätzen mit vielen Variablen kann die Funktion **`dplyr::glimpse()`** Euch helfen, zu verstehen, was im Datensatz überhaupt enthalten ist. Der Community Datensatz enthält geographische und zeitliche Variablen sowie Daten zur gesammelten Plastikmenge, Freiwilligen und Events:
...
grade_this_code()
```
Der **Audit-Datensatz** enthält also - wie der Community-Datensatz - die Gesamtplastikmenge, geographische und zeitliche Daten, aber keine Daten zu Events und Volunteers. Stattdessen findet Ihr hier Daten zu den Plastikarten.
...
Nun fügen wir dem Plot die **Datenpunkte** mit **`geom_point`** hinzu, um einen ersten Scatterplot zu erstellen. Diese Codezeile unterscheidt sich je nach gewählter Visualisierungsform. Theoretisch habt Ihr mit diesem Plot bereits einen Überblick über die Daten, wir werden den Plot im Folgenden aber noch etwas verfeinern.
...
ggplot2::ggplot(data = community, aes(x = continent, y = volunteers)) +
  geom_point(position = position_jitter(width = 0.3),
             size = 3,
             alpha = 0.6) # Hinzufügen der Datenpunkte inkl. Stylingoptionen zur Positionierung, Punktegröße und Transparenz
```
...
ggplot2::ggplot(data = community, aes(volunteers, continent)) +
...
#### Statistische Kennzahlen
Nun nutzen wir statistischen Kennzahlen, um mit Hilfe des `audit_plastic`-Datensatz die folgenden Fragen zu beantworten:
  ...
  3. Wieviel Plastikarten wurden gesammelt? <br>
...
quiz(
  ...
  question(
    "Wieviele Variablen hat der `audit_plastic`-Datensatz?",
  ...
  question(
    "Wieviele Länder sind im Datensatz?",
    answer("Das kann man anhand des Outputs nicht sagen."),
    answer("2019"),
  ...
  question(
    "Wieviele Kontinente sind im Datensatz?",
    answer("13"),
    answer("3"),
    answer(
      "Das kann man anhand des Outputs nicht sagen.",
      correct = TRUE
    ),
    answer("52"),
    correct = "Richtig!",
...
4. In der zweiten Einheit haben wir uns auf statistische Kennzahlen des Nutzt nun den `community` Datensatz und erstellt eine Tabelle, welche die Länder- und Teilnehmeranzahl je Kontinent darstellt.