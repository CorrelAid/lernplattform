...
- Parameter werden im **YAML-Header** von R Markdowns definiert
- Sie können über eine Anpassung des YAML-Header selbst oder innerhalb der **Renderfunktion**`rmarkdown::render("MyDocument.Rmd", params = ...)` adaptiert werden
...
quiz(caption = NULL,
  ...
  question("Wie können Parameter gesetzt werden?",
...
rmarkdown::render(
  ...
  params = list(start = "2019-01-01", ...) # Hier könnt Ihr die Parameterliste setzen
  )
```
...
purrr::walk(kontinent_liste, function(kontinent) {
  rmarkdown::render(
    ...
    params = list(kontinent = kontinent) # Hier könnt Ihr die Parameterliste setzen: Wir ändern nur den Parameter "Kontinent"
  )
```
