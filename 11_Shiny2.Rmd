...
![]() *add image of shinyapp (drawn)*

Im Idealfall haben wir also die die Visualisierung des Balkendiagramms aus der letzten Woche in der Mitte und nutzen den linken Part der App für Auswahlmöglichkeiten.

<!--#### Wie erstellen wir den `main part` der Shiny-App?-->

#### 2. Server ergänzen

Hierfür können wir unseren Code in die `server.R`-Datei kopieren und leicht anpassen. Machen wir das doch einmal zusammen. Die Grundstruktur des Servers sieht so aus:
...
Da wir in dieser Umgebung keine Shiny-App programmieren können, nutzen wir dazu eine lokale R-Datei. Öffnet dafür die Datei `app.R`. Ergänzt dort Code des Balkendiagramms im Server. 
