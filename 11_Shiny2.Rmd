...
quiz(caption = NULL,
  question("Welche Auswahlmöglichkeiten (oder auch Filteroptionen) habt ihr in der Shiny-App?",
    ...
    incorrect = "Leider falsch: Versuche es einfach nochmal oder schau dir noch einmal die Shiny-App genauer an",
    allow_retry = TRUE,
    try_again_button = "Versucht es nochmal"
  )
)
...
Wichtig ist, dass die `id` (hier `continentSelection`) nur einmal vergeben wird, damit wir es später im Server eindeutig zuordnen können.
...
quiz(caption = NULL,
  question("Wie bringt man in der Shiny-App den `server` und die `ui` zusammen?",
    ...
    correct = "Richtig! Und genau das setzen wir jetzt auch um :-)",
    incorrect = "Leider falsch: Versuche es einfach nochmal oder schau dir das Video noch einmal an",
    allow_retry = TRUE,
    try_again_button = "Versucht es nochmal"
  )
)
...
Wunderbar - setzt es doch gleich in Aufgabe 4 (`# 4. Aufgabe` in der `app.R`-Datei) um! Hier kurz zur Erinnerung wie der Code in der Datei aussehen sollte:
...
Jetzt sind wir endlich soweit -- lasst das gesamte Dokument einmal durchlaufen und probiert Eure Shiny-App auf Eurem Rechner aus. Was fällt Euch auf?
...
quiz(caption = NULL,
  question("Was fällt Euch auf, wenn Ihr Euch die Shiny-App lokal genauer anschaut?",
    ...
    incorrect = "Leider falsch: Versuche es einfach nochmal oder dir die Shiny-App noch einmal genauer an",
    allow_retry = TRUE,
    try_again_button = "Versucht es nochmal"
  )
)
