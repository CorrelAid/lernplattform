...
![](VIDEO) (ca. 15 Minuten) 
*Notiz an mich: Slide mit Beispielen bringen; kurz Server und UI erklären*
...
R liefert ein eigenes Package, `Shiny`, um eine Shiny-App zu bauen. In dem Video habt Ihr bereits die Grundkomponenten kennen gelernt, die jede Shiny-App hat: **eine UI** (kurz für "**u**ser **i**nterface", Englisch für Benutzeroberfläche) und **einen Server**.
...
<right>![*Gehirn](daten/abbildungen/brain.png){#id .class width=20% height=100%}</right>
...
<right>![*Körper](daten/abbildungen/body.png){#id .class width=20% height=100%}</right>
...
Wenn wir diese Abbildung in unsere Shiny-App integrieren wollen, können wir einfach den Code in unsere `server.R`-Datei kopieren und leicht anpassen. Aber machen wir das doch einmal zusammen. 
...
2. Öffnet RStudio und erstellt ein neues Projekt. Wählt dort "ShinyApp" aus.  Gebt eurer Shiny-App einen geeigneten Namen. Wählt dort die Variante mit den separaten Dateien (jeweils eine Datei für die UI (`ui.R`) und eine Datei für den Server (`server.R`)) aus. **todo: nochmal schauen wie es genau heißt**
...
4. Nehmt euren Code für die Visualisierung (wenn Ihr noch keinen Code habt erstellt zunächst eine Visualisierung mit `ggplot2` und nutzt diesen Code hier) und baut ihn in die `server.R`-Datei ein. Orientiert Euch hier an der Anleitung oben oder aber auch am [Schummelzettel]().
...
6. Wenn Ihr möchtet, könnt Ihr Eure Shiny-App über ein `theme` noch weiter verschönern. [Hier]() findet Ihr eine Liste an Auswahlmöglichkeiten. Um es umzusetzen **TBC**.

7. Und jetzt kommt der spannendste Part: Lasst Eure Shiny-App laufen! Führt dazu `shinyApp(ui = ui, server = server)` in der Console aus und probiert Eure Shiny-App aus!
