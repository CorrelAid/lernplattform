...
Zunächst betrachten wir das **Gehirn**, oder auch die Schaltzentrale der Shiny-App: den **Server**. Der `server` Datei kann so aussehen:
...
Hier ist die Shiny-App für Euch - klickt auf den Link und schaut Euch an, was der Code generiert hat:
...
![](){target="_blank"} **Add link to the app**

Probiert die App doch einfach aus. Wie Ihr sehen werdet, könnt Ihr innerhalb der Shiny-App nach Kontinenten filtern und wir sehen auch auf einmal, dass Ozeanien auch Freiwillige hat - eben nur sehr wenige!
...
Da wir auf dieser Plattform keine Shiny-App direkt programmieren können, haben wir Euch hier die Code-Snippets zusammengestellt und erklären Euch die einzelnen Schritte. Um die Shiny-App während der Sitzung mitzubauen, haben wir Euch die Datei `app.R` vorbereitet. **Cosima: VERLINKEN - sollen wir auch gleich auf app_loesung als Fallback-Option hinweisen?** Die Datei könnt Ihr Euch wie einen "Lückentext" vorstellen, an dem Ihr an den entsprechenden Stellen den Code einfügt und abschließend Eure Shiny-App lokal laufen lasst! 
...
Angewandt auf unseren Code wäre das wiefolgt:
...
Der Server nimmt zwei Argumente: `input` und `output`. Wichtig ist hier, dass wir alle erzeugten Komponenten der Shiny-App an `output` anhängen, um darauf später in der `ui` darauf zugreifen zu können. Wenn wir zum Beispiel eine Visualisierung erzeugen, können wir es `output$balkendiagramm` nennen.
...
Wenn Ihr mehr wissen möchtet, findet Ihr [hier eine gute Übersicht (auf Englisch)](https://mastering-shiny.org/basic-ui.html#outputs).
...
Sehr gut! Dann lasst uns doch die Auswahlmöglichkeit in unsere Shiny-App einbauen. Die Tabelle gibt Euch eine kurze Übersicht, über die wichtigsten Befehls- und Auswahlmöglichkeiten:

| Was möchtet Ihr dem Nutzer als Auswahlmöglichkeit geben? | Befehl in R (wird in der `ui` implementiert)                                                                                                                                                                    |
|----------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
...
Wichtig ist, dass alle Auswahlmöglichkeiten nach einem ähnlichen Schema aufgebaut sind (die Komplexität variiert natürlich abhängig von der Auswahlmöglichkeit, die Ihr gewählt habt):

```
checkboxGroupInput(

      # Hier definiert Ihr eine "id" (das ist wichtig, um den `server` mit der 
      # `ui`) später zu verbinden
      "continentSelection", 
                            
      # hier wird die Beschreibung eingefügt
      
      "Wählt einen Kontinent aus", 
      
      # Der nächste Schritt ist wichtig - hier definieren wir, was der
      # Nutzer auswählen kann. 
      # Dieser Schritt variiert von Auswahlmöglichkeit zu Auswahlmöglichkeit.
      # Die Logik ist, dass wir eine Liste (`list`) an Möglichkeiten in 
      # `choiches` speichern. Wir möchten hier alle Kontinente auswählen. 
      # Dazu müssen wir jeden Kontinent sein Anzeigeäquivalent (linke Seite) 
      ...
      # Oft gibt es dann auch die Möglichkeit eine Vorauswahl zu treffen, die 
...
- **`id`**: Wichtig ist, dass die `id` (hier `continentSelection`) nur einmal vergeben wird, damit wir es später im Server eindeutig zuordnen können.
- **Beschreibung**: Hier seid Ihr absolut frei und könnt eine Beschreibung auswählen, die Euch für den Nutzer passend erscheint
- **Auswahlmöglichkeiten**: Unter `choices` definieren wir hier zum Beispiel eine Liste (`list`), die uns die einzelnen Kontinente im Datensatz auswählen lässt (wenn Ihr es noch einmal überprüfen möchtet, könnt Ihr Sie Euch mit `table(community$continent)` anzeigen lassen). Was sehr praktisch ist, ist hier die Möglichkeit der Benennung innerhalb der Shiny-App (also was der Nutzer in der Shiny-App sieht). Was wir hier zum Beispiel als Möglichkeit nutzen können, um deutschsprachige Labels zu vergeben!
...
Wir haben im `server` unser Balkendiagramm in `output$balkendiagramm` gespeichert - das ist hier eine wichtige Information, da wir es hier mit `"balkendiagramm"` direkt ansprechen und einbinden können. Bearbeitet dazu Aufgabe 3 im Code in Eurer `app.R`-Datei.
...
Nach der Bearbeitung der Daten und vor der Erstellung der Visualisierung mit `ggplot2` haben wir die Möglichkeit noch einen zusätzlichen Filter einzubauen. Im Code wäre das an der gekennzeichnenten Stelle:
...
3. Danach erstellt Ihr Euch einen Account auf [ShinyApps.io](https://www.shinyapps.io){target="_blank"} (es gibt verschiedene Optionen, wählt den aus, der am besten zu euren Bedürfnissen passt)
...
1. Wählt Euch eine Visualisierung (z.B. mit `ggplot2` oder aber auch mit `echarts4r`)  mit euren Daten aus (idealerweise habt Ihr schon Code, den Ihr jetzt einfach wieder benutzen könnt - zum Beispiel aus der letzten Woche. Wenn Ihr keinen Code bereit habt, haben wir etwas für Euch vorbereitet **TODO: MUSS NOCH GEMACHT WERDEN - oder ist das zu viel? Ich würde eine app.R Datei erstellen und eine Visualisierung einbauen und die Teilnehmenden bauen dann darauf auf**)
