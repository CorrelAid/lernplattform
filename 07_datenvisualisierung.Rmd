...
Also wir stellen unsere Daten als "geometrische Objekte" mit "ästhetischen Eigenschaften" dar.  `ggplot2` folgt wie gesagt einer "Lagenlogik" oder "Schichtlogik", nach der eine Visualisierung Stück für Stück aufgebaut wird. Eine ganz einfache Visualisierung mit `ggplot2` erfolgt immer über folgendes Prinzip:
...
Wir sehen schon dass es hier einige Ausreißer nach oben gibt, was die Teilnehmerzahl & die gesammelte Menge angeht. Hier gibt es zwei Optionen:

1. Wir werfen diese Beobachtungen weg
2. Wir wählen eine passende Darstellungsform

Wir entscheiden uns für zweiteres & stellen unsere Daten auf einer logarithmischen Skala dar. Hierfür fügen wir neue Ebenen hinzu: `scale_x_log10()` für eine logarithmische x-Achse, und `scale_y_log10()` für eine logarithmische y-Achse:
...
Allerdings könnten jetzt sehr große Punkte kleinere verdecken! Eine weitere Eigenschaft der Punkte ist `alpha`, ihre Deckkraft. Wenn wir einer Eigenschaft einen festen Wert geben wollen, können wir ihn *außerhalb* von `aes()` zuweisen (entweder innerhalb `ggplot()` oder `geom_point()`):
...
ggplot(community, aes(x = n_volunteers, y = n_pieces, color = continent, size = n_events)) +
  ...
  theme_minimal()
```
Noch besser. Als nächstes sehen wir uns die Achsen an: die y-Achse ist aktuell nicht optimal formattiert (wir hätten gern 1,000 statt 1e+03) & am besten bringen wir die x-Achse ins selbe Format damit alles einheitlich ist. Das Format der labels können wir mithilfe des `scales`-packages innerhalb der entsprechenden Ebenen ändern:
...
final_plot <- 
  ...
  labs(
    ...
    subtitle = "More volunteers & events = more plastic collected?",
    x = "Number of Volunteers",
    y = "Number of pieces collected",
...
world <- st_read(here::here('daten/geospatial/ne_50m_admin_0_countries.shp'), quiet = TRUE)
world <- 
  world %>% 
  filter(SOVEREIGNT != "Antarctica") %>%
  # ^ werden wir wohl nicht brauchen & nimmt viel Platz ein...
  select("countrycode" = ISO_A2)
tibble(world)
```

In diesem Datensatz mit "simple features" haben wir neben dem `countrycode` noch eine weitere Spalte, `geometry`, in der die "Form" jedes Landes enthalten ist. In unseren Ursprungsdaten ist auch der `countrycode` der einzelnen Länder mit enthalten; den können wir benutzen um unsere Beobachtungen mit der Karte zu joinen:

```{r ggplot_maps_merging, exercise = TRUE, exercise.setup = "ggplot_maps_data"}
map <- 
  world %>% 
  left_join(community, by = "countrycode") %>% 
  select("country" = "countrycode", starts_with("n_")) %>%
  # NA in community = keine Events, kein Plastik gesammelt:
  mutate(across(starts_with("n_"), function(x) tidyr::replace_na(x, 0)))
```
...
In R lassen sich mithilfe des `plotly`-Packages auch interaktive Grafiken erstellen. Die einfachste Variante ist, einfach die `ggplotly()`-Funktion auf einen bestehenden ggplot anzuwenden, um diesen in einen interaktiven Plot zu konvertieren:
