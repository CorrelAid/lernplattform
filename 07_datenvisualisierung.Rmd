...
map <- world %>% 
  dplyr::left_join(community %>% select(-country), by = "countrycode") %>% 
  dplyr::select(country, countrycode, starts_with("n_")) %>%
  # NA in community = keine Events, kein Plastik gesammelt:
  dplyr::mutate(across(starts_with("n_"), function(x) tidyr::replace_na(x, 0)))
```
...
Die plotly-Plots könnt Ihr als HTML Datei speichern und dann im Browser öffnen:
