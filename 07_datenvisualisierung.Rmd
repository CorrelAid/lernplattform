...
- Wir schauen uns in dieser Lektion gemeinsam die R-Packages `ggplot2` und `plotly` (für interaktive Visualisierungen) an & werden kurz über das `sf`-Package sprechen, welches die Arbeit mit Geodaten ermöglicht (praktisch z.B. um Karten zu erstellen).
...
world <- 
  ...
  select("countrycode" = ISO_A2, "country" = SOVEREIGNT)
tibble(world)
```
...
map <- 
  world %>% 
  left_join(community %>% select(-country), by = "countrycode") %>% 
...
with_labels <- 
  map %>%
  rename("Land" = country, "Events" = n_events) %>%
...
with_labels %>% 
  ...
  ggplotly(tooltip = c("Land", "Events"))
```
