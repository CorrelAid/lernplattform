...
- Eine Shiny-App besteht immer aus einer **UI** und einem **Server**:
  - Die **UI** stellt das User Interface (dt. Benutzeroberfläche) oder auch den Körper der App dar.
...
<!-- @Review: Braucht man den Absatz mit in Kürze tatsächlich? Irgendwie ein wenig redundant nach der Zusammenfassung drüber und auch nicht so viel kürzer -->
...
<!-- @Review: Kommentar einfügen, dass das Ausführen des Codeblocks keine Shiny-App öffnen wird? Bzw. allgemeiner Disclaimer zu der Lektion, dass Code ausführen in der Lernplattform nicht wirklich zielführend ist und das lokal gemacht werden soll? -->
...
<!-- @Review: Das Objekt daten gibt es nicht, müsste wahrscheinlich noch im Setup der Lernplattform hinterlegt werden, dass die Visualisierung hier läuft -->
...
Probiert die App doch einfach einmal aus! Wie Ihr sehen werdet, könnt Ihr innerhalb der Shiny-App **nach Kontinenten filtern**. Die Übungsdateien könnt Ihr jetzt [hier](https://correlcloud.org/index.php/s/ngFQ79p6ciwdFoG){target="_blank"} herunterladen.
<!-- @Review: vielleicht direkt Link zum Unterordner, wo die Dateien liegen? --> 
...
Wenn Ihr mehr wissen möchtet, findet Ihr [hier eine gute Übersicht](https://mastering-shiny.org/basic-ui.html#outputs) (auf Englisch). Insbesondere wenn Ihr **Shiny-Erweiterungen** nutzt, müsst Ihr innerhalb der Erweiterung nach der richtigen Renderfunktion suchen. Schließlich benennen wir unseren Output `output$balkendiagramm`. 
...
<!-- ...oder wenn man Schritt 1 aufmerksam gelesen hat, vielleicht dort dann rausnehmen, sonst ist die Frage ein bisschen doppelt?-->
```{r 11filter}
quiz(caption = NULL,
...
| Einfachauswahl aus Liste                                 | `selectInput("id", "Beschreibung", auswahl)` <br> In `auswahl` könnt Ihr entweder die Auswahl definieren (so wie wir es in Übung 2 machen) oder Ihr greift auf ein anderes Objekt (in diesem Fall ` auswahl` zu)     |
...
![](https://github.com/CorrelAid/lernplattform/blob/main/daten/abbildungen/shinyapps1.png?raw=true){#id .class width=80% height=80%}

Dort klickt Ihr auf Euer Benutzerkonto (rechts oben) und dann auf "Tokens" und gelangt zu folgendem Fenster:

![](https://github.com/CorrelAid/lernplattform/blob/main/daten/abbildungen/shinyapps2.png?raw=true){#id .class width=80% height=80%}
...
![](https://github.com/CorrelAid/lernplattform/blob/main/daten/abbildungen/shinyapps3.png?raw=true){#id .class width=80% height=80%}
