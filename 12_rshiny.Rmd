...
<!-- Cosima: Ich sehe kein `daten` Objekt und nehme an, dass der Kommentar schon gelöst ist?-->
...
<!-- Cosima: Sollte es in der App nicht auch für die Konsistenz "Shiny-App" heißen? (Also bei "Eine Übung zum Erlernen von Shiny Web Apps mit echten Daten aus dem tidyverse von und für die Zivilgesellschaft mit CorrelAid e.V. Lizenziert nach CC-BY 4.0.") -->
...
<!-- Cosima: Soll ich uns wieder eine "handschriftliche" Skizze der App entwerfen, die wir hier einbinden können? -->
...
Für den Server unserer App können wir den Code für das Balkendiagramm an der richtigen Stelle in die Grundstruktur der `app.R`-Datei kopieren, eine passende Renderfunktion auswählen (Tipp: Schaut auf den [Schummelzettel](daten/cheat sheets/shiny_de.pdf){target="_blank"}!) und benennen. In unserer App ergibt das:
...
<!-- Cosima: Das ist damit gemeint, oder? Oder was sollen "Shiny-Erweiterungen" sein? Oder hab ich das damals geschrieben? :D Dann weiß ich es selbst nicht mehr und finde es so logischer :) -->
...
*Tipp: Genauso wie wir es hier gemacht haben, ist es oft leichter, zunächst in einer `Rmd.-Datei` notwendige Packages zu laden, die Daten zu importieren, zu bereinigen und zu Tabellen, Diagrammen und anderen Formaten zu verarbeiten. Im Anschluss könnt Ihr den dort funktionierenden Code in eine Shiny Web App übertragen.*

<!-- Cosima: Wenn ich das vor 6 Monaten so geschrieben habe, würde ich dem Punkt " Auch zur Fehlerbehebung ist dieses Vorgehen gut geeignet." heute nicht mehr zustimmen. Copy Paste verursacht so viele Fehler und ich würde auch nicht sagen, dass das wirklich Fehler an sich reduziert, deswegen habe ich es rausgenommen :) -->
...
<!-- Cosima: Sind Tabs nicht Reiter? (Oder Registerkarten, wie Google mir gerade sagt :D) -->
...
<!-- Cosima: Haben wir an dem Bild die Rechte? Wenn ich es damals reingenommen habe, würde ich es gerne wieder rausnehmen. Wenn ihr es gerne mit dabei habt, würde ich dafür votieren ein Bild im Corporate Design von uns zu entwerfen (ich übernehme das gerne) - dann können wir auch gleich die deutsche Bezeichnung übernehmen :) -->
...
<!-- Cosima: Sollten wir dieses Bild nicht auch ändern? Sonst ist es verwirrend, oder? Wir haben ja jetzt eine andere App. Sag gern Bescheid, dann passe ich es an. -->
...
<!-- Cosima: Ich bin nicht ganz sicher, was damit gemeint ist. Haben wir die Frage noch an einer anderen Stelle? :) --> 
```{r 11filter}
quiz(caption = NULL,
...
<!-- Cosima: Wenn ich mich richtig erinnere war es bei der Visualisierungssitzung ähnlich - es ist Absicht, dass die Slidedecks mit den Anschauungsmaterialen alle rausgenommen wurden? (Passt für mich auch; ich hatte mich nur gewundert, da sie uns ermöglichen den Teilnehmenden eine "Live"-Visualisierung der verschiedenen Möglichkeiten zu zeigen) -->
...
<!-- Cosima: Ich kann es optisch gerade nicht überprüfen - aber i.d.R. bietet diese "Zitieroption" die Möglichkeit, die Textstelle hervorzuheben. -->
