...
![*Video: Arbeiten mit Text (30min)*](tbd)
...
quiz(caption = NULL,
  ...
  question("Was kann man mit dem regulären Ausdruck `[A-ZÄÖÜ]{1,3}-[A-Z0-9]{1,2} \\d{1,4}` finden?",
    answer("Deutsche Postleitzahlen"),
    answer("Telefonnummern"),
    answer("Daten im Format 'TT.MM.JJJJ'"),
    answer("Deutsche KFZ-Kennzeichen", correct = TRUE),
    correct = "Richtig!",
    incorrect = "Leider falsch: Versuche es einfach nochmal oder schau im Video nach!",
    allow_retry = TRUE,
    try_again_button = "Nochmal versuchen"
...
Nutzt nun `str_to_title()` um die Variable `country` harmonisiert im sogenannten Title Case auszugeben (erster Buchstabe jedes Wortes jeweils großgeschrieben). Zur Erinnerung: Ecuador und Nigeria sind in Großbuchstaben codiert.
...
Als nächstes möchten wir die Namen einiger multinationaler Konzerne im Datensatz harmonisieren. Um den Code kurz und flexibel zu halten, nutzen wir hierzu `str_replace()` zusammen mit regulären Ausdrücken.
...
data_raw  |> 
  ...
  dplyr::mutate(
    ...
    parent_company = str_replace(parent_company, '.*[Pp]epsi.*', 'PepsiCo')
  )
```

Als weiteres Beispiel zur praktischen Anwendung schauen wir uns Kraft Foods an. Hierfür gibt es folgende Varianten:
...
data_raw |> 
  # ..und bearbeitet ihn mittels dplyr::mutate
  dplyr::mutate(
    parent_company = str_replace(parent_company, '.*[Kk]raft.*', 'Kraft Foods')
                )
```
...
regex_df |> 
  mutate(
    kfz_valide = str_detect(kfz, '[A-ZÄÖÜ]{1,3}-[A-Z0-9]{1,2} \\d{1,4}')
  )
```

Als nächstes erstellen wir als Übung eine neue Spalte `PLZ` aus den Daten der Spalte `adresse`. Hierfür können wir nutzen wir die Funktion `str_match()` und den geeigneten regulären Ausdruck. Die Funktion hat zwei Argumente und hat folgende Syntax: `str_match(*Text*, *Regex/Zeichenfolge*)`
...
Zunächst möchten wir das Werk mit der Funktion `unnnest_tokens()` in seine einzelnen Wörter zerlegen. Diese werden dabei auch standardmäßig in Kleinschreibung umgewandelt. Danach bereinigen wir den Datensatz um sogenannte Stoppwörter, Zahlen und einzelne Buchstaben. Zuletzt zählen wir, wie häufig die einzelnen Wörter im Text vorkommen und visualisieren dies dann in einer Wordcloud.
...
wordcloud(hitlist$wort, hitlist$n, min.freq = 20)
```
