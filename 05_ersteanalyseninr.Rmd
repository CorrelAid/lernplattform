...
Nun kennen wir den Grundaufbau des Datensatzes schon und möchten die Variablen, die für unsere Fragestellungen interessant sind besser verstehen. Dazu geben wir uns mit `base::summary()`` **erste (statistische) Eigenschaften** der Variablen aus.
...
Ebenso zur Datenexploration gehört die Visualisierung der Rohdaten. Dafür erstellen wir ein Punktediagramm (*engl. Scatterplot*), genauer: Einen Beeswarmplot (*dt. Bienenschwarmdiagramm*). Diese Visualisierung stellt sowohl die Verteilung als auch die Häufigkeit von Beobachtungen über die Verteilung dar. Um den Bezug zu statistischen Kennzahlen herstellen zu können, legen wir darüber noch **Boxplots**, die die Verteilung von Variablen inklusiver wichtiger statistischer Eigenschaften darstellen. Nehmt Euch einen Moment und beschreibt die Graphik in Euren Worten. Was fällt Euch auf?
...
ggplot(data = community, aes(x = continent, y = n_pieces, fill = continent)) + # Initialisierung des ggplots mit Variablen
  ggbeeswarm::geom_beeswarm(size = 3, alpha = 0.5, color = "darkgrey") + # # Hinzufügen der Datenpunkte (Scatterplot) inkl. Stylingoptionen zur Positionierung, Punktegröße, Transparenz und Farbe zur Verdeutlichung der Anzahl
...
Im ersten Schritt der Datenexploration fügen das **arithmetrische Mittel pro Kontinent** ein (als Dreieck gekennzeichnet):

```{r geom_point_n_pieces_bericht_mittelwert, exercise = TRUE}
# Erstellung eines Boxplots mit Punktewolke zur Anzahl gesammelter Plastikstücke pro Kontinent
ggplot(data = community, aes(x = continent, y = n_pieces, fill = continent)) + # Initialisierung des ggplots mit Variablen
  ggbeeswarm::geom_beeswarm(size = 3, alpha = 0.5, color = "darkgrey") + # Hinzufügen der Datenpunkte (Scatterplot) inkl. Stylingoptionen zur Positionierung, Punktegröße, Transparenz und Farbe zur Verdeutlichung der Anzahl
  #geom_boxplot(alpha = 0.6, width=.1, position = position_nudge(x = -0.25)) + # Hinzufügen des Boxplots
...
Der Boxplot fasst die fünf Punkte der Verteilung zusammen und gibt damit einen sehr guten Überblick über Daten. Diesen fügen wir nun zum Graphen mit dazu: 

```{r graphik_bericht_inkl_boxplot}
# Erstellung eines Boxplots mit Punktewolke zur Anzahl gesammelter Plastikstücke pro Kontinent
ggplot(data = community, aes(x = continent, y = n_pieces, fill = continent)) + # Initialisierung des ggplots mit Variablen
  ggbeeswarm::geom_beeswarm(size = 3, alpha = 0.5, aes(color = continent)) + # # Hinzufügen der Datenpunkte (Scatterplot) inkl. Stylingoptionen zur Positionierung, Punktegröße, Transparenz und Farbe zur Verdeutlichung der Anzahl
  geom_boxplot(alpha = 0.6, width=.1, position = position_nudge(x = -0.25)) + # Hinzufügen des Boxplots, seitliche Verschiebung, so dass es die Punktewolke nicht überlagert
  ...
  labs(
    ...
    y = "Anzahl gefundener Plastikstücke",
    x = "Kontinent",
    caption = glue::glue("N = {nrow(community)} \nDatenquelle: TidyTuesday und BFFP")) + # Festlegung der Achsenbezeichusngen, Überschriften und Titel
  theme_minimal() + # Festlegung des Layout-Designs  
  theme(legend.position="none") + # Ausblenden der Legende
  scale_fill_manual(values = c("#C9DFE6", "#94C0CD", "#4E97AC", "#366978", "#2E5A67"))+ # Anwendung der BFFP-Farben
...
So lautete unsere Daten-Frage. Diese können wir nun beantworten. Nach der Datenexploration wissen wir unter anderem, dass wir dem allgemeinsprachlichen "durchschnittlich" mit einem Vergleich der Mediane begegnen werden. Betrachtet nun nochmals das Diagramm oben und antwortet auf die Quizfragen. 
...
<!-- RE: Hier wäre Median + IQR wahrscheinlich am Saubersten -->
<!-- Sylvi: das habe ich im graphen mal so implementiert -->


Bisher haben wir uns vorallem der *univariaten* Verteilung von Plastikstücken gewidment. Nun möchten wir die Daten weiter nutzen, um *multivariat* herauszuarbeiten: Welche Faktoren beeinflussen möglicherweise die Unterschiede in der Anzahl an Plastikstücken, die gesammelt wurden? Wir schauen uns an, wie die Freiwilligenzahl und die Zahl an Events mit der Plastikstücken Zahl zusammenhängt. 


```{r scatter_plot_n_volunteers}

# Erstellung einer Punktewolke zur Anzahl gesammelter Plastikstücke und Freiwilligenzahl
ggplot(data = community, aes(x = n_volunteers, y=n_pieces, fill = continent)) + # Initialisierung des ggplots mit Variablen
  geom_point(size = 3, alpha = 0.5, aes(color = continent)) + # # Hinzufügen der Datenpunkte (Scatterplot) inkl. Stylingoptionen zur Positionierung, Punktegröße, Transparenz und Farbe zur Verdeutlichung der Anzahl
   coord_cartesian(ylim = c(0, median(community$n_pieces) + 1.5* IQR(community$n_pieces)),
                   xlim = c(0, median(community$n_volunteers) + 1.5 * IQR(community$n_volunteers))) + # Festlegung der Achsenlänge der y-Achse abhängig von Median und IQR
geom_vline(xintercept=median(community$n_volunteers), color= "darkgray") +
geom_hline(yintercept=median(community$n_pieces), color= "darkgray") +
  labs(
    title = "Zusammenhang zwischen gesammelten Plastikstücke und Anzahl Freiwilliger",
    y = "Anzahl gefundener Plastikstücke",
    x = "Anzahl Freiwilliger",
    caption = glue::glue("Ausreißer zur besseren Lesbarkeit ausgeklammert. \nN = {nrow(community)}, Datenquelle: TidyTuesday und BFFP")) + # Festlegung der Achsenbezeichusngen, Überschriften und Titel
  theme_minimal() + # Festlegung des Layout-Designs  
  theme(legend.position="top") + # Ausblenden der Legende
  scale_fill_manual(values = c("#C9DFE6", "#94C0CD", "#4E97AC", "#366978", "#2E5A67")) + # Anwendung der BFFP-Farben
  scale_color_manual(values = c("#C9DFE6", "#94C0CD", "#4E97AC", "#366978", "#2E5A67")) #
```

In diesem Scatterplot haben wir zwei Hilfslinien abgetragen, welche jeweils den Median Wert erfassen. Durch die Hilflinien entstehen 4 Quadranten. Im Quadranten links unten und rechts oben fallen die meisten Punkte. Dies deutet auf einen positiven statistischen Zusammenhang hin. Denn je größer die Anzahl an Freiwilligen desto größer oftmals auch die Anzahl an gesammelten Plastikstücken. Wir sehen allerdings auch, dass links unten recht viele Punkte geballt auftreten. Es gibt also schon auch recht viele Länder, die im internationalen Vergleich eine geringe Anzahl Freiwilliger und eine geringe Anzahl an Plastikstücken aufzuweisen haben. 

Der Korrelationskoeffizient bringt diesen statistischen Zusammenhang auf den Punkt in einer Zahl. Berechnet ihn für diese beiden Variablen.

```{r korrelation_pieces_volunteers, exercise=TRUE}
cor(community$n_pieces, community$n_volunteers)
```

0.81 bestätigt, dass wir einen positiven und recht starken statischen Zusammenhang zwischen der Anzahl an Plastikstücken und Freiwilligen beobachten. Beachtet, der Korrelationskoeffizient bietet keinerlei Möglichkeit einer kausalen Aussage. Werder unser Diagramm noch die der Koeffizient lassen den Schluss zu, dass alleinig mehr Freiwillige zu mehr gesammelten Plastikstücken führen.

```{r scatter_plot_n_events}

# Erstellung einer Punktewolke zur Anzahl gesammelter Plastikstücke und Freiwilligenzahl
ggplot(data = community, aes(x = n_events, y = n_pieces, fill = continent)) + # Initialisierung des ggplots mit Variablen
  geom_point(size = 3, alpha = 0.5, aes(color = continent), position = position_jitter(width = 0.1, height=0.1)) + # Hinzufügen der Datenpunkte (Scatterplot) inkl. Stylingoptionen zur Positionierung, Punktegröße, Transparenz und Farbe zur Verdeutlichung der Anzahl
   coord_cartesian(ylim = c(0, median(community$n_pieces) + 1.5* IQR(community$n_pieces)),
                   xlim = c(0, median(community$n_events) + 1.5 * IQR(community$n_events))) + # Festlegung der Achsenlänge der y-Achse abhängig von Median und Standardabweichung
  geom_vline(xintercept=median(community$n_events), color= "darkgray") +
  geom_hline(yintercept=median(community$n_pieces), color= "darkgray") +
  labs(
    title = "Zusammenhang zwischen gesammelten Plastikstücke und Anzahl Events",
    y = "Anzahl gefundener Plastikstücke",
    x = "Anzahl Events",
    caption = glue::glue("N = {nrow(community)} \nDatenquelle: TidyTuesday und BFFP")) + # Festlegung der Achsenbezeichusngen, Überschriften und Titel
  theme_minimal() + # Festlegung des Layout-Designs  
...
Seht euch nun den zweiten Scatterplot an, der die Anzahl gesammelter Plastikstücke und die Anzahl an Events gegenüber stellt.
...
Die meisten Länder haben tatsächlich nur ein Event veranstaltet. Dies bedeutet auch, es gibt in der Variable `n_events`nicht so viel Variation. Der Korrelationskoeffizient hier beträgt `r cor(community$n_pieces, community$n_events)`. Er ist zwar positiv, aber sehr nah bei null.

Was nehmen wir nehmen wir aus dieser bivariaten Analyse mit? Die Zahl an Freiwilligen scheint ein bedeutenderer Faktor zu sein, als die Anzahl an Events, die veranstaltet werden. Dies wäre ein Feedback, das in die Community zurück gespielt werden könnte für die nächsten Runden von **"Break Free from Plastic"**.

Kommen wir zum Schluss noch einmal zur allgemeinen Frage: Wie erfolgreich war der Audit? Wenn wir uns dieser Frage im Sinne von wie viel Plastik wurde gesammelt nähern. Dann gibt es durchaus recht große Unterschiede zwischen und innerhalb den Kontinenten. Als Einflussfaktor haben wir eher die Freiwilligenanzahl als die Anzahl an Events ausgemacht.
...
1. Überlegt: Wie könnte man die Frage Wie erfolgreich war der Audit? noch operationalisieren und in Datenfragen umwandeln? Wie könnte eine Visualisierung oder eine zusammenfassende Statistik dabei helfen? Skizziert Eure Fragen gerne schriftlich.
...
3. In der ersten Einheit haben wir uns bei der Visualisierung vor allem der **n_pieces Variable** gewidmet. Nun blicken wir auf die **n_volunteers**: Wie sehr unterscheiden sich die Freiwilligenzahlen nach Kontinenten? Erstellt in dem heruntergeladenen RMarkdown ein **Punktediagramm** (Scatterplot) mit dem Datensatz `community` für diesen Blickwinkel auf den Erfolg der "Break Free from Plastik" Aktion. Die Graphik soll `n_volunteers`, die **Anzahl der Freiwilligen** auf der y-Achse und die **Kontinente** auf der x-Achse zeigen. Hinweis: versucht dazu im  RMarkdown in der finalen Version der Graphik die entsprechenden Variablen auszutauschen.
