...
Fügen wir nun das **arithmetische Mittel pro Kontinent** (als Dreieck gekennzeichnet) und die **Standardabweichung** (als Strich gekennzeichnet) in die Visualisierung ein. Hattet Ihr den Mittelwert an dieser Stelle erwartet? 
...
quiz(caption = NULL,
  ...
  question(
    "Die Standardabweichung beschreibt die positive und negative Schwankung um den Mittelwert. Was irritiert Euch bei der Betrachtung der Graphik?",
    answer("Die Standardabweichung ist auf den Kontinenten unterschiedlich groß."),
    answer("In Afrika und Asien scheint die Standardabweichung sehr hoch zu sein.", correct = TRUE),
    answer("Die Anzahl an Plastikstücken kann nicht negativ werden.", correct = TRUE),
    correct = "Richtig! Deshalb sollten wir hier die Daten auf Ausreißer untersuchen und ein Koordinatensystem mit natürlichen Nullpunkt nutzen.",
    incorrect = "Leider falsch: Die Standardabweichungen können über verschiedenen Kategorien schwanken, bei sehr hohen Werten kann dies aber auch Ausreißern (und nicht nur einer hohen Variation) geschuldet sein. Auch existiert für die Anzahl der Plastikstücke ein natürlicher Nullpunkt. Es können nicht -3 Plastikstücke gesammelt werden.",
    allow_retry = TRUE,
    try_again_button = "Nochmal versuchen",
    random_answer_order = TRUE
  ))
...
Er ist zwar positiv, aber sehr nah bei null. Die Trendlinie im Graphen ist nahezu waagerecht. 
...
Was nehmen wir nehmen wir aus dieser bivariaten Analyse mit? Die Anzahl an Events korrelieren nur sehr schwach positiv mit der Anzahl gesammelter Plastikstücke. Die Anzahl der Freiwilligen korreliert stark positiv mit der Anzahl an gesammelten Plastikstücken. Für diesen Zusammenhang kann es aber auch andere Ursachen geben, die wir in unseren Daten überhaupt nicht betrachten, zum Beispiel die Bevölkerungsanzahl. Welche "Störfaktoren" fallen Euch noch ein?
...
3. Versucht, das zugehörige [**R Markdown: 05_ErsteDatenanalysenInR **](https://correlcloud.org/index.php/s/ngFQ79p6ciwdFoG){target="_blank"} zum Laufen zu bringen und es nachzuvollziehen.

4. In der ersten Einheit haben wir uns bei der Visualisierung vor allem der **n_pieces Variable** gewidmet. Nun blicken wir auf die **n_volunteers**: Wie sehr unterscheiden sich die Freiwilligenzahlen nach Kontinenten? Erstellt in dem heruntergeladenen RMarkdown ein **Punktediagramm** (Scatterplot) mit dem Datensatz `community` für diesen Blickwinkel auf den Erfolg der "Break Free from Plastik" Aktion. Die Graphik soll `n_volunteers`, die **Anzahl der Freiwilligen** auf der y-Achse und die **Kontinente** auf der x-Achse zeigen. 
