...
Intro (5min)
1. Vorstellung
2. Motivation für R-Markdown Reports
  - keine Copy und Paste Fehler beim kopieren der Ergebnisse
  - automatisieren möglich (Bsp. CFE)
  - viele Outputs möglich
3. Lernziele
  - Woche 1 Modularprinzip: Grundkenntnisse R Markdown
  - Woche 2 Reportfabrik: Reports Automatisieren
  - generell Überblick: was ist alles möglich?
  
Inhalt
1.  Kurze Einführung/Wiederholung: Was ist R-Markdown?\
    1. Zitate
    2. Zusammenfassung
    3. Gallery + Tabellenwettbewerb Beispiel

2.  Struktur von RMarkdown: Wie sind die Komponenten aufgebaut?  
    1.  YAML Header\
    2.  Inhalte\
    3.  Knitr Code Chunks\

4.  Inhalte: Wie kann ich den Text formatieren?  
    1.  Überschriften\
    2.  Text\
    3.  Listen\
    4.  Links\
...
-->
![*Video: Reports in RMarkdown (30min)*](https://youtu.be/YWFLSovbha0)
...
*Text in italics* <br>
erstellt mit: \*Text in italics\*

_________________________________________________________________

**Text in bold** <br>
erstellt mit: \*\*Text in bold\*\*

_________________________________________________________________

##### Einfügen eines Bildes
![Bild: Logo CorrelAid](https://betterplace-assets.betterplace.org/uploads/organisation/profile_picture/000/033/251/crop_original_bp1613490681_Logo.jpg){#id .class width=10% height=10%} <br>
erstellt mit:
```{r, reportformatpic, exercise = TRUE}
'
![Bildbeschriftung](Link zum Bild){#id .class width=10% height=10%}
'
```
*Anmerkung: Damit unsere Lernplattform den Code nicht interpretiert, haben wir ihn hier in einem Code Chunk auskommentiert. Den Snippet könnt Ihr (ohne '') kopieren und in den weiß hinterlegten Teil Eures RMDs einpflegen. Mit "!" könnt Ihr das Bild einbetten. Breite und Höhe werden hier relativ angegeben.*

_________________________________________________________________

##### Einfügen eines Hyperlinks:
[Hyperlink: Webseite CorrelAid](www.correlaid.org){target="_blank"}<br>
erstellt mit:
```{r, reportformatlink, exercise = TRUE}
'
[Hyperlinkname](Link){target="_blank"}
'
```
...
Code Blöcke werden in Eurem RMDs **grau hinterlegt**. Ihr könnt sie mit
...
\```{r, argument1 = gewünschte_einstellung, ...} <br>
Dein Code hier <br>
\``` <br>
...
knitr::opts_chunk$set(argument1 = gewünschte_einstellung, ...) <br>
\``` <br>
...
```{r 08quiz_reports_codechunksettings}
quiz(caption = NULL,
  ...
  question("Ausnahmsweise soll nun einmal der Code gezeigt werden. Was müsst Ihr tun?",
    answer("Im betroffenen Code Chunk setzen wir das Argument echo auf TRUE"),
    ...
    answer("Im betroffenen Code Chunk setzen wir das Argument include auf TRUE", correct = TRUE),
    correct = "Richtig!",
    incorrect = "Leider falsch: Mit dem Argument include wird definiert, ob Code und Code Output in das Outputdokument integriert werden sollen. Dieses muss auf TRUE gestellt werden, wenn das geschehen soll, und auf auf FALSE, wenn es nicht geschehen soll.",
    allow_retry = TRUE,
    try_again_button = "Nochmal versuchen"
  )
)
...
### Und jetzt Ihr
1. **Lasst Euch inspirieren**: Besucht die [R-Studio R Markdown Galerie](https://rmarkdown.rstudio.com/gallery.html) und stöbert durch einige Beispielrports, die mit R Markdown erstellt wurden. Ihr könnt zwischen Webseiten, PDF-Dokumenten oder Word-Dokumenten wählen. Teilt Euer Lieblingsbeispiel (als Screenshot und/oder mit Link) im Slack Channel und beschreibt in 1-3 Sätzen, was Ihr an dem Report besonders gelungen findet.
2. Bearbeite lokal die [Übung](){target="_blank"} zu R Markdown, die wir für Euch vorbereitet haben.