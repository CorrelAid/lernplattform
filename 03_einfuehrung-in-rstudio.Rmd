...
- Im **Code Editor** (zu dt. Codebearbeitungsprogramm) wird programmiert - den Code führt Ihr mit "CMD + ENTER" für Mac und "CTRL + ENTER" für Windows aus
...
#### R Markdown (Rmd)
- Ein **Code Chunk** kann in RStudio in RMDs mit \```{r} Dein Code hier\```, über das Menü "Code -> Insert Chunk" oder den Kurzbefehlen "CMD + CTRL + i" für Mac und "CTRL + Alt + i" für Windows) hinzugefügt werden
...
- Mit dem `here`-Package und der Funktion `here` (ja, im Code ergibt sich damit ein `here::here()`), könnt Ihr Euer Arbeitsverzeichnis dann auf den Ordner setzen, in dem die .Rproj-Datei gespeichert ist und relativ dazu auf alle anderen Dateien zugreifen (z.B. *here::here('data/plastics.csv')*)
...
quiz(caption = NULL,
  ...
  question("Du möchtest die Funktion \"filter\" aus dem \"dplyr-Package\" verwenden. Wie gehst Du vor?",
    answer("Ich installiere zuerst das Package über \"library(dplyr)\"."),
    answer("Nachdem das Package über \"install.packages('dplyr')\" installiert ist, muss ich es mit \"library(dplyr)\" noch laden.", correct = TRUE),
    answer("Wenn das Package installiert und geladen ist, kann ich die Funktion im Code verwenden: \"filter::dplyr()\""),
        answer("Wenn das Package installiert und geladen ist, kann ich die Funktion im Code verwenden: \"dplyr::filter()\"", correct = TRUE),
    correct = "Richtig!",
...
Einen Überblick über die Grundlagen in R erhaltet Ihr in diesem [**Schummelblatt**](https://correlaid.org/material/Schummelblatt_baseR.pdf){target="_blank"}. In diesem Abschnitt stellen wir Euch die wichtigsten Grundlagen für die kommenden Wochen vor! 
...
Was für Daten gibt es denn eigentlich? Klassisch denkt Ihr vermutlich an Zahlen, aber Text und ein binäres TRUE/FALSE sind genauso Informationen. In R könnt Ihr mit den verschiedenen Grundtypen an Daten arbeiten:
...
Neben einzelnen Werten lässt sich natürlich auch eine ganze Reihe solcher Werte in einem Objekt speichern. Hierbei handelt es sich um einen Vektor, d.h. alle einzelnen Werte werden über das `c()` zusammengepackt. Achtung: in einem Vektor sollten nur Werte des gleichen Grundtyps gespeichert sein!
...
Listen sind die Grundlage von Datensätzen, die schauen wir uns jetzt in der interaktiven Übung an!
...
Ein **Codechunk** (zu dt. Codeblock), also ein Abschnitt mit Code, der Pakete installiert und lädt, sieht so aus. Hier wurde der Code auskommentiert (da wir alle Pakete, die für die Lernplattform notwendig sind für Euch schon installiert und geladen haben). **Kommentare** erkennt Ihr in der Regel an dem Hash **"#"**.
...
Mit der Notation **`package::funktion()`** kennzeichnen wir, woher eine Funktion kommt und können - unabhängig von geladenen Package-Libraries - Funktionen nutzen. Dabei ist es zwar nicht nötig, das Package vor die Funktion zu schreiben - aber definitiv best practice! Die Funktionen folgen dem Syntax Funktion(argument1, argument2, ...). Dateien können u.a. mit dem Package **`rio`** und der Funktion **`import()`** geladen werden. Die geladenen Daten werden dann in einem Dataframe gespeichert - hier wird dem Datensatz der Name "data_raw" zugewiesen ("objektname <- objekt"). Über diesen ist er dann im weiteren Skript abrufbar.
...
![](abbildungen/03_einfuehrung-in-rstudio/forloop.png){#id .class width=30% height=100%}
...
![](abbildungen/03_einfuehrung-in-rstudio/whileloop.png){#id .class width=30% height=100%}
