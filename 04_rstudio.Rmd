...
- Innerhalb von Code muss Text auskommentiert werden (z.B. über das Einfügen eines Hashtags "#" zu Beginn des Textes)
...
quiz(caption = NULL,
  ...
  question("Welche Aussagen über den Syntax in R und RMDs sind wahr?",
    ...
    answer("Obejkte werden mit \"name <- objekt\" definiert.", correct = TRUE),
  ...
  question("Knitting bedeuted, dass ich aus einem ganzen RMD PDF-, HTML- oder Worddateien erstelle, während \"Run\" bedeutet, dass Code über die Konsole oder in meinem RMD interpretiert werden soll.",
    answer("Korrekt", correct = TRUE),
    answer("Inkorrekt"),
    correct = "Richtig!",
...
Wenn Ihr mit Eurem Datenprojekt startet, solltet Ihr Euch überlegen, welche **Funktionalitäten** Ihr braucht, um Daten zu importieren, zu bereinigen und zu analyiseren. Die benötigten Funktionalitäten müsst Ihr nicht alle selbst programmieren, dafür gibt es sogenannte **Packages** (zu dt. Pakete) - eine ziemlich gute Sache. Eure Arbeit beginnt deshalb in der Regel mit der **Installation und dem Laden** dieser Funktionalitäten. Dazu nutzen wir die Funktionen **install.packages("package")** und **library(package)**, wobei letztere auch im Script erhalten bleiben kann. Die Nutzung von Anführungszeichen ist nur bei der Installation (also für install.packages()) relevant. Bitte denkt daran, "install.packages()" auszukommentieren, wenn die Installation abgeschlossen ist - sonst wird die Installation bei jedem Ausführen des Skripts wieder durchgeführt.
...
Mit der Notation "package::funktion()" kennzeichnen wir, woher eine Funktion kommt und können - unabhängig von geladenen Package-Libraries - Funktionen nutzen. Die Funktionen folgen dem Syntax Funktion(argument1, argument2, ...). CSV-Dateien können u.a. mit dem Package **"readr"** und der Funktion **"read_csv()"** geladen werden. Die geladenen Daten werden dann in einem Dataframe gespeichert - hier wird dem Datensatz der Name "plastics" zugewiesen ("objektname <- objekt"). Über diesen ist er dann im weiteren Skript abrufbar.
Wenn Ihr Daten aus lokalen Ordnern laden möchtet, müsst Ihr den Link durch den Pfad der Datei (einsehbar über Rechtsklick auf den Dateinamen) auf Eurem Rechner ersetzen. Leichter wird das mit der Funktion **"here::here()"**, die Euch den Pfad in den Ordner zurückgibt, in dem auch Euer R-Skript abliegt.
...
### Exkurs (optional): Syntax in R
Neben dem sog. Variable Assignment über "<-" sind noch einige weitere **Syntaxregeln, Operatoren und Ausdrücke** in R wichtig. Diese lernt man in der Regel durch wiederholte Praxis. Damit Ihr die wichtigsten Formen nachschauen könnt, haben wir für Euch diesen Exkurs erstellt.
...
Mit **logischen Ausdrücke**n könnt Ihr u.a. Datensätze filtern und Wenn-Dann-Begingungen für Operationen definieren. Wenn der Ausdruck wahr ist, wird die Operation durchgeführt, wenn er falsch ist, dann wird sie übersprungen. Was denkt Ihr, was für die folgenden Ausdrücke gilt? Führt den Code Chunk aus und prüft Euer logisches Denken!
...
Falls Code nur unter bestimmten Bedingungen ausgeführt werden soll, wird er in **Wenn-Dann-Begingungen** integriert. Welcher Text wird hier gedruckt?
...
Wenn Ihr einzelne Daten, Zeilen, Spalten oder **Subsets** (zu dt. Teilmengen) eines Datensatzes braucht, könnt Ihr den Datensatz **"slicen"**. 
``` {r slicing1, exercise = TRUE}
# Auslesen des Datums in der ersten Zeile und ersten Spalte
plastics[1,1]
# Auslesen der ersten Zeile
plastics[1,]
# Auslesen der ersten Spalte
plastics[,1]
# Auslesen der ersten zwei Zeilen und Spalten
plastics[1:2,1:2]
...
subset(plastics, country == "Argentina" & year == 2020)
```
Aufgepasst: Es ist ganz normal, dass Euer Code mal **Fehler** wirft - auch uns passiert das. In der Console (und in RMDs unter dem Code Chunk) wird Euch angezeigt, **wo der Fehler auftritt und um welche Art von Fehler es sich handelt**. Beliebte Fehlerquellen sind **Syntaxfehler** (wenn Ihr beispielsweise Klammern nicht schließt oder Argumente nicht durch Kommata trennt) oder die falsche Verwendung von Argumenten in Funktionen. Auch das Schreiben von Kommentaren in Code Chunks ist nur mit "#" zu Beginn des Textes möglich. Wenn Ihr mal gar nicht weiter wisst, kopiert einfach die Fehlermeldung aus der Console und begebt Euch damit auf eine Googlesuche. In **StackOverflow** finden sich für (fast) alle Herausforderungen Antworten und Hilfestellungen.
